...
class GitHubApi : GitForgeApi {
    ...
    override suspend fun listFiles(token: String, repo: String, dir: String): List<GitFile> {
        return try {
            val (code, body) = request("GET", "https://api.github.com/repos/$repo/contents/$dir", token)
    ...
    override suspend fun getFileContent(token: String, repo: String, path: String): String? {
        return try {
            val (code, body) = request("GET", "https://api.github.com/repos/$repo/contents/$path", token)
    ...
    override suspend fun putFile(token: String, repo: String, path: String, content: String, sha: String?): String? {
        return try {
            ...
            val (code, body) = request("PUT", "https://api.github.com/repos/$repo/contents/$path", token, bodyObj.toString())
    ...
    override suspend fun deleteFile(token: String, repo: String, path: String, sha: String): Boolean {
        return try {
            ...
            val (code, _) = request("DELETE", "https://api.github.com/repos/$repo/contents/$path", token, bodyObj.toString())
            code == 200
        } catch (e: Exception) {
            false
        }
